...
async def create_device():
    ...
    if request.method == "POST":
        ...
        if (
            form["is_vacuum"] == "0"
            and form["drupal_name"].strip().replace(" ", "") != ""
        ):
            db.execute(
        ...
        db.execute(
            ...
            [
                form["name"].strip().replace(" ", "").replace("-", "_"),
                ecrypt_pass(),
                ota_pass(),
                form["has_reader"],
                form["is_vacuum"],
...
async def create_reader_to_tool():
    ...
    if request.method == "POST":
        ...
        db.execute(
            ...
            [form["reader"], form["tool"], str(datetime.now())],
        )
        db.commit()
...
async def create_tool_to_vacuum():
    ...
    if request.method == "POST":
        ...
        db.execute(
            ...
            [form["tool"], form["vacuum"], str(datetime.now())],
        )
        db.commit()
...
# ===================================== UPDATE ==================================
# update device
@crud_gui.route("/update/device/mac_addr", methods=["POST"])
async def update_device_mac_addr():
    # logging.info("visited /update/device")
    db = toolauth._get_db()
    form = await request.form
    value = form["value"].replace(" ", "")
    id = form["id"].replace(" ", "")
    logging.info("Update MAC address || value: " + value + " | id: " + id)
    db.execute(
        """UPDATE base
              SET mac_addr=(?)
            WHERE id=(?);""",
...
# update device
@crud_gui.route("/update/device/ip_addr", methods=["POST"])
async def update_device_ip_addr():
    # logging.info("visited /update/device")
    db = toolauth._get_db()
    form = await request.form
    ...
    value = form["value"].replace(" ", "")
    id = form["id"].replace(" ", "")
    logging.info(form.__dict__)
    logging.info("Update IP address || value: " + value + " | id: " + id)
    db.execute(
        """UPDATE base
              SET ip_addr=(?)
            WHERE id=(?);""",
